...
public void actionPerformed(ActionEvent e){
	if (e.getSource()==startA){
		...
		tre = new Thread(){
			...
			giveUp.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					tre.stop();
					new AddScore("Addition",uName, right, total);
					tym.setText("Seconds Left: 60");
					numL.setText("");
					eq.setText("");
					tf.setEditable(false);
					p3.removeAll();
	...
	if (e.getSource()==tf){
		try{
		if (Integer.parseInt(tf.getText())==ans){
			System.out.println(ans+"right");
			right++;
			}

		else if (!(Integer.parseInt(tf.getText())==ans))
