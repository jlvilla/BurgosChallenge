...
class GUISub extends JFrame implements ActionListener {
	private JPanel p,p1, p2, p3;
	private JButton startA, giveUp, exit;
	private JLabel tym, seq, numL;
	private JTextField tf, eq;
	private int seconds, ans, right, total, ave, leftNum, rightNum;
	private double scored, num;
	private String uName;
	private Thread tre;

public GUISub(String name){
	super("Subtraction");
	uName = name;
	p = new JPanel();
	p1 = new JPanel();
	...
	if (e.getSource()==startA){
		...
		tre = new Thread(){
			...
			giveUp.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					tre.stop();
					new AddScore("Subtraction",uName, right, total);
					total = 0;
					right = 0;
					ans = 0;
					seconds = 60;
