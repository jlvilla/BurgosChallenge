...
class GUIDiv extends JFrame implements ActionListener {
	...
	if (e.getSource()==startA){
		...
		tre = new Thread(){
			...
			giveUp.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent e){
					tre.stop();
					new AddScore("Division",uName, right, total);
					total = 0;
					right = 0;
					ans = 0;
					seconds = 60;
...
public void quest(){
	tf.requestFocus();
	tf.setText("");
	leftNum = (int)(Math.random()*100 + 1);
	rightNum = (int)(Math.random()*12 + 1);
	while (leftNum%rightNum!=0 || leftNum==0 || rightNum==0){
		leftNum = (int)(Math.random()*100 + 1);
		rightNum = (int)(Math.random()*12 + 1);
		}
